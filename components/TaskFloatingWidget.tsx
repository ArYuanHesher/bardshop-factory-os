  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      ...
      {isOpen && (
        <div className="mb-4 w-96 h-[500px] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-up">
          ...
          <div className="bg-slate-950 p-4 border-b border-slate-800 flex justify-between items-center">
            {activeTab === 'chat' ? (
              <button onClick={() => setActiveTab('list')} className="text-slate-400 hover:text-white flex items-center gap-1 text-sm transition-colors">
            ...
            <div className="flex gap-3 items-center">
               <Link href="/tasks" className="text-xs font-bold text-blue-500 hover:text-blue-400 transition-colors">開啟完整看板</Link>
              ...
              <div className="divide-y divide-slate-800">
                {tasks.length === 0 ? (
                    ...
                    <div 
                        ...
                        className={`p-4 cursor-pointer hover:bg-slate-800/50 transition-colors ${!task.is_read ? 'bg-blue-900/10 border-l-2 border-blue-500' : 'border-l-2 border-transparent'}`}
                    >
                        ...
                        <div className="mt-2 flex items-center gap-2">
                            <span className={`text-[10px] px-1.5 py-0.5 rounded border ${
                                task.status === 'returned' ? 'border-red-500/30 text-red-400 bg-red-900/10' : 
                                task.status === 'completed' ? 'border-green-500/30 text-green-400 bg-green-900/10' : 
            ...
            {activeTab === 'chat' && selectedTask && (
              <div className="p-4 space-y-4">
                 <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                    <h3 className="font-bold text-white text-sm mb-2">{selectedTask.title}</h3>
                    <p className="text-xs text-slate-300 leading-relaxed">{selectedTask.content}</p>
                 </div>
                 
                 <div className="space-y-3">
                    <div className="text-center text-[10px] text-slate-600 my-4">— 快速預覽模式 (僅顯示摘要) —</div>
                 </div>

                 <div className="mt-4 pt-4 border-t border-slate-800">
                    <Link href="/tasks" className="block w-full py-3 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded-lg font-bold text-center transition-colors">
                       進入任務看板回覆
                    </Link>
                 </div>
              </div>
      ...
      <button 
        ...
        className="group relative flex items-center justify-center w-14 h-14 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full shadow-2xl hover:scale-110 transition-transform active:scale-95 border border-white/10"
      >
        ...
        {!isOpen && unreadCount > 0 && (
          <span 
            className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 border-2 border-[#050b14] rounded-full text-white text-[10px] font-bold flex items-center justify-center transition-transform duration-300 ease-out origin-bottom-left shadow-md z-10"
            style={{ 
              transform: `scale(${scale})` // 動態應用計算出的縮放比例
            }}
          >
